# Asset Credits

This file documents the source and license for all game assets.

## Summary

All game sprites are sourced from [Kenney.nl](https://kenney.nl) and are licensed under **CC0 1.0 Universal (Public Domain)**. No attribution is required, but we credit Kenney here because they make amazing free game assets.

---

## Kenney.nl Asset Packs

### Furniture Kit
- **Source**: [Kenney Furniture Kit](https://kenney.nl/assets/furniture-kit)
- **License**: CC0 1.0 (Public Domain)
- **Author**: Kenney (kenney.nl)

**Assets used (86 items):**

| Category | Assets |
|----------|--------|
| Furniture | furniture_sofa_blue, furniture_sofa_cream, furniture_sofa_basic, furniture_sofa_cozy, furniture_sofa_modern, furniture_armchair_plush, furniture_armchair_vintage, furniture_bed_single, furniture_bed_double, furniture_desk_wooden, furniture_desk_modern, furniture_desk_simple, furniture_desk_vintage, furniture_table_coffee, furniture_table_side, furniture_table_dining, furniture_table_glass, furniture_bookshelf_tall, furniture_bookshelf_short, furniture_bookshelf_antique, furniture_wardrobe, furniture_dresser, furniture_dresser_vintage, furniture_chair_dining, furniture_chair_wooden, furniture_chair_modern, furniture_stool, furniture_plant_stand, furniture_reading_chair, furniture_tv_stand, furniture_cabinet, furniture_nightstand |
| Decor | decor_plant_fern, decor_plant_monstera, decor_plant_small, decor_plant_medium, decor_plant_large, decor_plant_hanging, decor_plant_succulent, decor_lamp_floor, decor_lamp_floor_arc, decor_lamp_table, decor_lamp_warm, decor_lamp_modern, decor_lamp_antique, decor_vase_tall, decor_plant_cactus, decor_basket_woven, decor_cushion_set, decor_pumpkin_small, decor_pumpkin_large, decor_jack_o_lantern, decor_candelabra, decor_blanket_knit, decor_cocoa_station |
| Rugs | rug_basic, rug_round_cream, rug_circular_soft, rug_modern_geo, rug_vintage_persian, rug_sheepskin, rug_autumn_leaves, rug_spiderweb, rug_winter_pattern, rug_rectangular, rug_soft_square |
| Wall | wall_poster_simple, wall_art_abstract, wall_clock_round, wall_clock_basic, wall_mirror_oval, wall_shelf_floating, wall_photo_frame, wall_tapestry |
| Surface Decor | surface_mug_coffee, surface_mug_cocoa, surface_book_stack, surface_book_open, surface_books, surface_candle_small, surface_candle_set, surface_plant_tiny, surface_photo_frame, surface_lamp_small, surface_vase_small, surface_typewriter |
| Outdoor | outdoor_table_bistro, outdoor_table_small, outdoor_planter_large, outdoor_planter_small, outdoor_flower_bed, outdoor_bird_bath, outdoor_chair_basic |

---

### Holiday Kit
- **Source**: [Kenney Holiday Kit](https://kenney.nl/assets/holiday-kit)
- **License**: CC0 1.0 (Public Domain)
- **Author**: Kenney (kenney.nl)

**Assets used (13 items):**

| Category | Assets |
|----------|--------|
| Decor | decor_string_lights, decor_tree_holiday, decor_wreath_door |
| Wall | wall_autumn_wreath, wall_stockings |
| Surface Decor | surface_snow_globe, surface_gingerbread |
| Outdoor | outdoor_snowman, outdoor_string_lights |
| Wall Finishes | wall_finish_cream, wall_finish_sage, wall_finish_burnt_orange, wall_finish_navy, wall_finish_white |
| Floor Finishes | floor_finish_warm_wood, floor_finish_light_wood, floor_finish_tile_white, floor_finish_carpet_grey, floor_finish_stone |

---

### Graveyard Kit
- **Source**: [Kenney Graveyard Kit](https://kenney.nl/assets/graveyard-kit)
- **License**: CC0 1.0 (Public Domain)
- **Author**: Kenney (kenney.nl)

**Assets used (22 items):**

| Category | Assets |
|----------|--------|
| Decor | decor_pumpkin_small, decor_pumpkin_large, decor_jack_o_lantern, decor_candelabra |
| Surface Decor | surface_mug_coffee, surface_mug_cocoa, surface_mug, surface_candle_small, surface_candle_set, surface_skull_candle, surface_sculpture |
| Outdoor | outdoor_bench_wooden, outdoor_lantern_tall, outdoor_lights_path, outdoor_path_stones, outdoor_hay_bale, outdoor_scarecrow, outdoor_tombstone, outdoor_ghost_sheet, outdoor_fire_pit |
| Rugs | rug_spiderweb |
| Wall | wall_bat_garland |

---

### Mini Dungeon
- **Source**: [Kenney Mini Dungeon](https://kenney.nl/assets/mini-dungeon)
- **License**: CC0 1.0 (Public Domain)
- **Author**: Kenney (kenney.nl)

**Assets used (1 item):**

| Category | Assets |
|----------|--------|
| Surface Decor | surface_potion_bottles |

---

### Fantasy Town Kit
- **Source**: [Kenney Fantasy Town Kit](https://kenney.nl/assets/fantasy-town-kit)
- **License**: CC0 1.0 (Public Domain)
- **Author**: Kenney (kenney.nl)

**Assets used (4 items):**

| Category | Assets |
|----------|--------|
| Wall | wall_poster_simple, wall_art_abstract, wall_frame_ornate |
| Outdoor | outdoor_bird_bath |

---

### Mini Forest
- **Source**: [Kenney Mini Forest](https://kenney.nl/assets/mini-forest)
- **License**: CC0 1.0 (Public Domain)
- **Author**: Kenney (kenney.nl)

**Assets used (1 item):**

| Category | Assets |
|----------|--------|
| Outdoor | outdoor_tree_small |

---

## License Text

### CC0 1.0 Universal (Public Domain Dedication)

The Kenney assets are released under CC0 1.0:

> No Copyright
>
> The person who associated a work with this deed has dedicated the work to the public domain by waiving all of his or her rights to the work worldwide under copyright law, including all related and neighboring rights, to the extent allowed by law.
>
> You can copy, modify, distribute and perform the work, even for commercial purposes, all without asking permission.

Full license: https://creativecommons.org/publicdomain/zero/1.0/

---

## Phase 4: Avatar & Wearables

Avatar sprites are currently generated as simple shape placeholders. When real LPC (Liberated Pixel Cup) sprites are imported, they will be palette-swapped for skin/hair color variants.

### Phase 4 Avatar Placeholders (Generated)
- **Source**: Generated via `scripts/build-avatar-atlas.ts`
- **License**: CC0 1.0 (Public Domain) - Original work by Homestead
- **Author**: Homestead

**Generated assets (132 sprites):**

| Category | Variants | Count |
|----------|----------|-------|
| Body/Skin | 6 tones × 2 poses | 12 |
| Eyes | 4 colors × 2 poses | 8 |
| Hair | 3 styles × 8 colors × 2 layers × 2 poses | 96 |
| Starter Tops | 3 colors × 2 poses | 6 |
| Starter Bottoms | 3 colors × 2 poses | 6 |
| Starter Shoes | 2 colors × 2 poses | 4 |

### Phase 4 Wearables - Future (LPC Attribution)
When LPC sprites are imported, they will be credited here:

- **Source**: [Liberated Pixel Cup](https://lpc.opengameart.org/)
- **License**: CC-BY-SA 3.0 / GPL 3.0 (dual-licensed)
- **Authors**: Various contributors (see `LICENSES/LPC-CREDITS.txt`)

**Planned wearables (54 items):**

| Category | Assets |
|----------|--------|
| Starter Tops | starter_tee_white, starter_tee_grey, starter_tee_black |
| Starter Bottoms | starter_pants_blue, starter_pants_black, starter_pants_khaki |
| Starter Shoes | starter_sneakers_white, starter_sneakers_black |
| Hair Styles | hair_pixie, hair_curly, hair_ponytail, hair_braids, hair_bun, hair_mohawk, hair_shaved, hair_wavy_long |
| Tops | top_sweater_cozy, top_hoodie, top_jacket_denim, top_blouse_floral, top_tshirt_graphic, top_cardigan, top_tank, top_dress_casual, top_flannel, top_crop, top_turtleneck, top_coat_winter, top_sweater_holiday |
| Bottoms | bottom_jeans_dark, bottom_shorts, bottom_skirt_mini, bottom_skirt_long, bottom_joggers, bottom_leggings, bottom_cargo, bottom_overalls |
| Shoes | shoes_boots_ankle, shoes_sandals, shoes_loafers, shoes_boots_hiking, shoes_slippers, shoes_heels |
| Accessories | acc_glasses_round, acc_glasses_square, acc_sunglasses, acc_beanie, acc_cap, acc_scarf, acc_earrings, acc_headband, acc_witch_hat, acc_bunny_ears, acc_flower_crown |

---

## Additional Assets

### Phase 4 Placeholder
- **bodydouble_placeholder** - Placeholder sprite for focus timer avatar (to be replaced)

---

## Support Kenney

While Kenney assets are free, consider supporting their work:
- Website: https://kenney.nl
- Patreon: https://patreon.com/kenney
- Donate: https://kenney.nl/donate
